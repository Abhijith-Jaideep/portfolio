...
export interface CaseStudy {
  ...
  media: {
    ...
    /**
     * Collapsed card image. For phone media it stops the thumbnail
     * autoplaying; for diagram media it lets the card show something
     * recognisable when the in-card screenshot is a detail crop.
     */
    thumbnail?: string;
  };
}
...
export const caseStudies: CaseStudy[] = [
  ...
  },
  {
    ...
    architecture: [
      ...
      "Sensor feed pulled in a single bulk export call, with the paged reader kept as a fallback so the map degrades to slow rather than broken",
    ...
    highlights: [
      "Cut map load from roughly 20 seconds to about one by profiling the bottleneck: reading 6,324 sensors through a 100-row endpoint meant 64 sequential requests, replaced with a single bulk export",
      ...
      "Cached by how often each thing actually changes: occupancy for two minutes, restriction rules for fifteen",
    ],
    outcome: [
      "Live at parkingbae.me, serving 4,851 bays matched to their restriction rules",
      "Built against a live council data feed rather than fixtures",
      "Rebuilt in 2026 after the original free-tier database was deleted: schema recreated, restriction data reloaded from City of Melbourne open data, and the fleet chart re-sourced from BITRE",
    ],
    stack: [
      "React",
      "Tailwind",
  ...
  },
  {
    ...
    media: {
      ...
      thumbnail: "/images/commentsanitizer-landing.webp",
      screenshots: [
        {
          src: "/images/commentsanitizer-comments.webp",
          caption: "Comment thread, every entry cleared by the model first",
        },
      ],
    },
  },
